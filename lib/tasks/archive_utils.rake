...
namespace :db do
  namespace :archive do
    desc "Archive Jobs"
    task :jobs, :needs => :environment do |t, args|
      hosts = {}
      puts "-----------------------------------------------------------------"
      puts "-- #{Time.now} rake db:archive:jobs starting"
      puts "-- #{Time.now} Environment '#{Rails.env}'"
      finished_jobs = Job.where(:exit_status => 0)
      ...
      puts "-- #{Time.now} rake db:archive:jobs finished"
      puts "-----------------------------------------------------------------"
    end
  end
end
